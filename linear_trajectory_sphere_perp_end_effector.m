...
%% Robot Creation
% Initial configuration obtained from the teach function
theta=[28,0,-90,180,90,-150]/180*pi;
% DH parameters
a2=0.4318;
a3=0.0202;
d3=0.1244;
...
% Create robot
PUMA_cw=SerialLink(Links,'name','Mic PUMA');
...
for p = a:e;
    ...
    %Creates homogeneous transformation matrix from rotation & position matrix
    T{p,1}= rt2tr(rotation, pos_static_trajectory(:,p));  
    %Concatenates the trajectories into one matrix for easier use
    if p==a
        T_Traj = T{p,1};
    else
...
%Initialise empty matrix for joint angles
q_init = NaN(a-1,6);
